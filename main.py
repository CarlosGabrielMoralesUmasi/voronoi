...
if __name__ == '__main__':
    ...
    points = []
    triangles = []
    const = 1000000.0  # Constante utilizada para calcular infinitos puntos lejanos de Voronoi
    ...
    # Trazado de puntos y triángulos (con duplicados)
    ...
    # Agregar Triángulos más cercanos a cada Triángulo
    for i in range(0, len(triangles)):
    ...
    # Resolviendo centro de triángulo por 3 puntos (dos rectas perpendiculares)
    ...
    # Punto de retorno de la línea ortogonal que va al infinito (depende de la variable const)
    def infpoint(p1, p2, p3, center):
        ...
        # Cálculo de turnos para facilitar la comparación
        dx = 0
        dy = 0
        if p1.x > p2.x:
            dx = p1.x - p2.x
    ...
    for i, t in enumerate(triangles):  # Generando aristas de Voronoi + vértices
        if t.t1 == None:
    ...
    if type_intr == 1:  # Case 1 Nearest Neighbor
        temp_dist = const
        temp_trgl_id = None
        for cell in Voronoi_vertices:  # Mira a qué celda el nuevo punto está más cerca
            if temp_dist > distance(newpoint, cell[0]):
                temp_dist = distance(newpoint, cell[0])
                temp_trgl_id = cell[1]
        temp_dist = const
        if temp_dist > distance(newpoint, triangles[temp_trgl_id].p1):  # Comprobando cada punto en el triángulo que encontramos
            temp_dist = distance(newpoint, triangles[temp_trgl_id].p1)
            newpoint.z = triangles[temp_trgl_id].p1.z
        if temp_dist > distance(newpoint, triangles[temp_trgl_id].p2):  # Comprobando cada punto en el triángulo que encontramos
            temp_dist = distance(newpoint, triangles[temp_trgl_id].p1)
            newpoint.z = triangles[temp_trgl_id].p2.z
        if temp_dist > distance(newpoint, triangles[temp_trgl_id].p3):  # Comprobando cada punto en el triángulo que encontramos
            temp_dist = distance(newpoint, triangles[temp_trgl_id].p1)
            newpoint.z = triangles[temp_trgl_id].p3.z

    if type_intr == 2:  # Case 2 
        temp_dist = const
        temp_trgl_id = None
        for cell in Voronoi_vertices:  # Mira a qué celda el nuevo punto está más cerca
            if temp_dist > distance(newpoint, cell[0]):
    ...
    if type_intr == 4:  # Case 4 Plano
        temp_dist = const
        tti = None  # Temp triangulo id
        for cell in Voronoi_vertices:  # Mira a qué celda el nuevo punto está más cerca
            if temp_dist > distance(newpoint, cell[0]):
                temp_dist = distance(newpoint, cell[0])
                tti = cell[1]
        ...
        abc = p1p2 * p1p3  # Creando a b c del plano
        d = -(abc[0] * triangles[tti].p1.x + abc[1] * triangles[tti].p1.y + abc[2] * triangles[tti].p1.z)  # d del plano
        newpoint.z = -(abc[0] * newpoint.x + abc[1] * newpoint.y + d) / (abc[2])  # Insertar nuevo punto en función del plano
